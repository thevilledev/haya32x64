...
MSVC, on x86 ILP32, and on a native AArch64 runner. The header is compiled as
C++11/C++17, a short libFuzzer job checks one-shot versus streaming splits,
and pull requests run SMHasher3 `VerifyAll`. Tag releases wait for this
workflow, so alignment, byte order, state layout, and compiler ABI
